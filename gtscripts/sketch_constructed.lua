...
gene   = gt.genome_feature_new(seqid, "gene", 100, 900, "+")
exon   = gt.genome_feature_new(seqid, "exon", 100, 200, "+")
gene:add_child(exon)
intron = gt.genome_feature_new(seqid, "intron", 201, 799, "+")
gene:add_child(intron)
exon   = gt.genome_feature_new(seqid, "exon", 800, 900, "+")
gene:add_child(exon)
nodes[1] = gene

-- construct a single-exon gene on the reverse strand
-- (within the intron of the forward strand gene)
reverse_gene = gt.genome_feature_new(seqid, "gene", 400, 600, "-")
reverse_exon = gt.genome_feature_new(seqid, "exon", 400, 600, "-")
