...
owntestfiles = ["#{$testdata}../ourtests/0.01/0.01.fas",
                "#{$testdata}../ourtests/0.1/0.1.fas",
                "#{$testdata}../ourtests/varsize/varlen.fas",
                "#{$testdata}../ourtests/varsize/varlen_2.fas"]
...
Name "gt condenser owntest"
Keywords "gt_condenser extratest owntest"
Test do
  opt_arr.each do |opt|
    owntestfiles.each do |file|
      basename = File.basename(file)
      run_test "#{$bin}gt encseq encode -indexname #{basename} " +
        "#{file}"
      run_test("#{$bin}gt condenser compress " + opt +
      ...
      run_test "#{$bin}gt condenser extract " +
        "-original #{basename} " +
        "#{basename}_nr > ext_c"
      run_test "#{$bin}gt encseq decode -output fasta #{basename} > ext_e"
      run "diff ext_c ext_e"
    end
  end
end
...
Test do
  file = "#{$testdata}../ourtests/simple/small_10_10b.fas"
  basename = File.basename(file)
  run_test "#{$bin}gt encseq encode -indexname #{basename} " +
    "#{file}"
