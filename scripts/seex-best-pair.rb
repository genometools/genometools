...
def resultfile_gen(tag,seedlength,minlength,errperc,extra)
  return "RESULTS/result-" + 
         [tag,seedlength,minlength,errperc,extra].join("-") + ".matches"
end
...
Inputparams = Struct.new("Inputparams",:errperc,:permathistory,:xdropbelow)
...
1.upto(maxerrperc).each do |errperc|
  ...
  [50,55].each do |permathistory|
    resultfile = resultfile_gen(taglist[0],seedlength,minlength,errperc,
                                permathistory)
    ...
    if gencall
      ...
      key = Inputparams.new(errperc,permathistory,nil)
      STDERR.puts "# seedhash1: size = #{h2.length}"
      seedhash1[key] = h1
    end
  end
  1.upto(maxxdropbelow).each do |xdropbelow|
    resultfile = resultfile_gen(taglist[1],seedlength,minlength,errperc,
                                xdropbelow)
    if gencall or runrepfind
    ...
    if gencall
      ...
      key = Inputparams.new(errperc,nil,xdropbelow)
...
differencelist.sort {|x,y| cmpdifference(x,y)}.each do |df|
  puts [df.inputparam.errperc,df.inputparam.permathistory,
