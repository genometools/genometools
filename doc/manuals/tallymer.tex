...
\SFX-index. When combined with option \Showoption{indexname},
this option specifies an occurrence constraint on the $k$-mers stored
in the generated \Tallymer-index. That is, a $k$-mer is put into the
\Tallymer-index, if it occurs #2\xspace $c$ times in the union of 
all sequences from the \SFX-index.
}
...
\Option{esa}{\Showoptionarg{\SFX-index}}{
Specify the name of an \SFX-index computed by the 
...
\Option{scan}{}{
In the default case, the \SFX-index is mapped into main memory. This means
that the program the index must not be larger than the available
...
\textsf{suf}-table of the \SFX-index are
...
\SFX-index. If this option is missing, then the default value for $k$ is 20.
}
...
and \Showoption{maxocc} in the \TYmkindex-index
...
\Option{counts}{}{
Specify that besides the \Tallymer-index a file
\Showoptionarg{idxname}\Filenamesuffix{.mct}
is generated storing the
counts of the $k$-mers represented by the mer-index. This option
...
of sequences represented by a \SFX-index. It is called as follows:
\par
...
\Tallymer-index. \TYsearch is called as follows:
\par
\noindent\GT \Programname{tallymer} \TYsearch [\emph{options}] -tyr
tallymer-index -q \emph{queryfile0} \emph{queryfile1} \dots
\par
where \emph{tallymer-index} is an index generated by \TYmkindex, and
\emph{queryfile0}, \emph{queryfile1}, etc.\ are queryfiles (in \Fasta format)
which are to be matches against the given index. The following options are 
available:

\begin{Justshowoptions}
\Option{tyr}{\Showoptionarg{\Tallymer-index}}{
Specify the name of a \Tallymer-index computed by the 
