...
\item
The openning braces (\keyword{\{}) after the keywords \keyword{if},
...
be ommitted, if the expression and the (single) following statement both fit on
a single line.
\item
...
\item
Use the \keyword{scripts/pre-commit} git hook to automatically run a \keyword{src_check}
...
\item
Static variables inside functions are not allowed. An exception are class structs, which
must be static.
\item
...
develpers employed by it, because only in this cases the University gains any
copyrights.
\end{itemize}
...
\item Add a \keyword{gt_<classname>_ref()} method to the interface of the class:

\begin{lstlisting}
GtClass* gt_class_ref(GtClass *c)
{
  gt_assert(c);
  c->reference_count++;
  return c;
...
\begin{lstlisting}
void gt_class_delete(GtClass *c)
{
  if (!c) return;
...
With reference counted classes, always use the \keyword{_ref()} method when
storing a reference to an object. That is, instead of writing

\begin{lstlisting}
void gt_myclass_set_value(GtMyclass *myclass, GtClass *c)
{
  myclass->value = c;
}
\end{lstlisting}
...
void gt_myclass_set_value(GtMyclass *myclass, GtClass *c)
{
  myclass->value = gt_class_ref(c);
}
\end{lstlisting}

Always remember to call \keyword{gt_<classname>_delete()} when the reference is
no longer needed! For example, in the assignment above, a good place to do this
is the destructor of the \keyword{GtMyclass} class.
...
environment. To fail the test manually, use the \keyword{failtest(msg)} command,
where \keyword{msg} is the error message to fail with.
\end{itemize}
...
\item
\keyword{dist}, which creates a tarball with a binary \Gt distribution,
\item
\keyword{srcdist}, which creates a tarball with a source \Gt distribution,
\item
\keyword{clean}, which removes all files created during the build and test
processes, except the \keyword{lib} and \keyword{bin} contents, and
\item
...
For \Gt development, we use the Git distributed versioning system\footnote{For
...
Pragmatic Bookshelf, ISBN 1934356158.} to track changes and
...
often based on the workstation's hostname, which may not be correct in many
cases.

Use the \keyword{git config} commands within your \Gt repository to set them
to a correct value:

\begin{lstlisting}
$ git config user.name "Hans Mustermann"
$ git config user.email "mustermann@zbh.uni-hamburg.de"
\end{lstlisting}
