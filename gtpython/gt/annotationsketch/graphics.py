...
class Graphics:
  ...
  def draw_box(self, x, y, width, height, fillcolor, astatus, awidth, swidth,  \
               scolor, dashed):
    if dashed:
      d_int = 1
    else:
      d_int = 0
    gtlib.gt_graphics_draw_box(self.g, x, y, width, height, fillcolor, astatus,\
                               awidth, swidth, scolor, d_int)

  def draw_dashes(self, x, y, width, height, astatus, awidth, swidth, scolor):
    gtlib.gt_graphics_draw_dashes(self.g, x, y, width, height, astatus, awidth,\
                                  swidth, scolor)

  def draw_caret(self, x, y, width, height, astatus, awidth, swidth, scolor):
    gtlib.gt_graphics_draw_caret(self.g,x, y, width, height, astatus, awidth,  \
                                 swidth, scolor)

  def draw_rectangle(self, x, y, filled, fcolor, stroked, scolor, swidth,      \
                     width, height):
    if filled:
      f_int = 1
    else:
    ...
    if stroked:
      ...
      s_int = 0
    gtlib.gt_graphics_draw_rectangle(self.g, x, y, f_int, fcolor, s_int,       \
                                     scolor, swidth, width, height)
  ...
  def draw_curve_data(self, x, y, color, data, ndata, valrange, height):
    ...
    for i in range(0, ndata):
      cdata[i] = data[i]
    gtlib.gt_graphics_draw_curve_data(self.g, x, y, color, cdata, ndata,       \
                                      valrange, height)
  ...
  def register(cls, gtlib):
    from ctypes import c_char_p, c_void_p, c_int, POINTER, c_double, c_ulong
    gtlib.gt_graphics_draw_text.argtypes = [c_void_p, c_double, c_double,      \
                                            c_char_p]
    gtlib.gt_graphics_draw_text_right.argtypes = [c_void_p, c_double, c_double,\
                                                  c_char_p]
    gtlib.gt_graphics_draw_text_centered.argtypes = [c_void_p, c_double,       \
                                                     c_double, c_char_p]
    gtlib.gt_graphics_draw_colored_text.argtypes = [c_void_p, c_double,        \
                                                    c_double, Color, c_char_p]
    ...
    gtlib.gt_graphics_set_margins.argtypes = [c_void_p, c_double, c_double]
    ...
    gtlib.gt_graphics_draw_horizontal_line.argtypes = [c_void_p, c_double,     \
                                                       c_double, Color,        \
                                                       c_double, c_double]
    gtlib.gt_graphics_draw_box.argtypes = [c_void_p, c_double, c_double,       \
                                           c_double, c_double, Color, c_int,   \
                                           c_double, c_double, Color, c_int]
    gtlib.gt_graphics_draw_dashes.argtypes = [c_void_p, c_double, c_double,    \
                                              c_double, c_double, c_int,       \
                                              c_double, c_double, Color]
    gtlib.gt_graphics_draw_caret.argtypes = [c_void_p, c_double, c_double,     \
                                             c_double, c_double, c_int,        \
                                             c_double, c_double, Color]
    gtlib.gt_graphics_draw_rectangle.argtypes = [c_void_p, c_double, c_double, \
                                                 c_int, Color, c_int, Color,   \
                                                 c_double, c_double, c_double]
    gtlib.gt_graphics_draw_arrowhead.argtypes = [c_void_p, c_double, c_double, \
                                                 Color, c_int]
    gtlib.gt_graphics_draw_curve_data.argtypes = [c_void_p, c_double, c_double,\
                                                  Color, c_void_p, c_ulong,    \
                                                  Range, c_ulong]
    gtlib.gt_graphics_save_to_file.restype = c_int
    gtlib.gt_graphics_save_to_file.argtypes = [c_void_p, c_char_p, Error]
