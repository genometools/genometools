...
static int canvas_lua_new_generic(lua_State *L, GraphicsOutType t)
{
  ...
  if (lua_isnil(L, 2))
    *canvas = canvas_cairo_file_new(style, t, width, NULL);
  else
  {
    ii = check_imageinfo(L, 2);
   *canvas = canvas_cairo_file_new(style, t, width, *ii);
  }
...
static int canvas_lua_to_file(lua_State *L)
{
  ...
  assert(canvas);
  ccf = (CanvasCairoFile*) *canvas;
  had_err = canvas_cairo_file_to_file(ccf, fn, err);
  if (had_err)
