...
/* generated by scripts/gen-esa-bottomup.sh: */
    ...
    unsigned long spaceforbucketprocessing, GtError *err)
{
  ...
  do
  {
    ...
    state->w_count = 0;
    if (gt_esa_bottomup_RAM_spmeq(seqnum_relpos_bucket, snrp, lcptab_bucket,
          nonspecials, (GtArrayGtBUItvinfo_spmeq *) state->stack, state, err)
        != 0)
    {
      return -1;
    }
    ...
    unsigned long spaceforbucketprocessing, GtError *err)
{
  ...
  do
  {
    ...
    state->w_overflow = false;
    state->w_count = 0;
    if (gt_esa_bottomup_RAM_spmvar(seqnum_relpos_bucket, snrp, lcptab_bucket,
          nonspecials, (GtArrayGtBUItvinfo_spmvar *) state->stack, state, err)
        != 0)
    {
      return -1;
    }
