...
typedef struct
{
  GtBitcount_type bits_kmerpos, /* sum of code_bits, bits_seqnum,
                                   bits_endpos, score_bits */
                  code_bits,
                  bits_seqnum,
                  bits_endpos,
                  score_bits;
  size_t bytes_kmerpos, bytes_code;
  GtUword first_seqnum, transfer_mask, mask_seqnum, mask_endpos,
          mask_score_bits;
  int code_shift, u_seqnum_shift, u_shift_endpos,
      b_seqnum_shift, b_endpos_shift, bits_left_adjust_score;
...
typedef struct
{
  ...
  uint8_t *bytestring;
  GtUword allocated, nextfree, differentcodes;
  const GtKmerPosListEncodeInfo *encode_info;
  uint32_t longest_code_run;
  unsigned int numofchars;
  GtRadixsortinfo *radixsortinfo_ulong;
} GtKmerPosList;
...
#define GT_DIAGBANDSEED_SAME_DIAGONAL(PREV,CURR)\
        (PREV).bpos == (CURR)->bpos
...
#define GT_DIAGBANDSEED_CONV_B(APOS,BPOS)\
        (seedpairlist->maxmat_diagonalchain \
...
struct GtDiagbandseedInfo
{
  ...
  bool norev,
       ...
       fullseedpairsort,
       onlykmers;
  GtUword a_sample_distance, b_sample_distance;
...
static GtDiagbandseedBaseListType gt_diagbandseed_base_type(size_t bytes,
                                                  ...
                                                  bool with_kenv)
{
  if (bt == GT_DIAGBANDSEED_BASE_LIST_UNDEFINED || with_kenv)
  {
                                               ...
                                               *extp)
{
  ...
  if (sample_kmers)
  {
    gt_assert(extp->userdefinedleastlength >= (GtUword) info->seedlength);
    info->a_sample_distance
      = (GtUword) ceil(sqrt(extp->userdefinedleastlength -
                            info->seedlength + 1));
    gt_assert(info->a_sample_distance > 1);
    info->b_sample_distance
       = (aencseq == bencseq) ? info->a_sample_distance
                             : (info->a_sample_distance-1);
    if (verbose)
    {
      printf("# sample distances " GT_WU "/" GT_WU "\n",
                info->a_sample_distance,
                info->b_sample_distance);
    }
  } else
  {
    info->a_sample_distance = info->b_sample_distance = 1;
  }
  info->verify_seeds = verify_seeds;
  ...
  info->fullseedpairsort = fullseedpairsort;
                                            ...
                                            *encode_info)
{
  ...
  if (encode_info != NULL && encode_info->bytes_kmerpos <= sizeof (GtUword))
  {
    kmerpos_list->radixsortinfo_ulong
      = gt_radixsort_new_ulong(0,encode_info->bytes_code);
  } else
  {
    kmerpos_list->radixsortinfo_ulong = NULL;
  }
  if (init_size > 0)
  {
    if (encode_info != NULL)
...
static void gt_kmerpos_list_delete(GtKmerPosList *kmerpos_list)
{
  ...
  {
    gt_radixsort_delete(kmerpos_list->radixsortinfo_ulong);
...
static GtCodetype gt_kmerpos_ulong_code(GtUword value,int code_shift)
{
                                    ...
                                    const GtKmerPosListEncodeInfo *encode_info)
{
  ...
  dec->code = gt_kmerpos_ulong_code(value,encode_info->code_shift);
...
static void gt_kmerpos_bytestring_encode(uint8_t *bytestring,
                                         ...
                                         GtUword seqnum,
                                         GtUword endpos,
                                         int bits_left_adjust_score,
                                         GtUword bpos_score_diff)
{
  ...
  value_seqnum_endpos
    = (seqnum << encode_info->b_seqnum_shift) |
      ...
      (bpos_score_diff << bits_left_adjust_score);
...
static GtUword gt_kmerpos_bytestring_code(const uint8_t *bytestring,
                                          int code_shift)
{
...
static uint8_t gt_kmerpos_bytestring_decode(GtDiagbandseedKmerPos *dec,
                                                 ...
                                                 *encode_info)
{
  ...
  dec->endpos = (v2 >> encode_info->b_endpos_shift) &
                encode_info->mask_endpos;
  return (v2 >> encode_info->bits_left_adjust_score) &
         encode_info->mask_score_bits;
}
...
#undef SKDEBUG
      ...
      gt_kmerpos_bytestring_encode(bytestring,
                                   ...
                                   0);
#ifdef SKDEBUG
      (void) gt_kmerpos_bytestring_decode(&kmerpos_entry2,bytestring,
                                          encode_info);
...
static void gt_kmerpos_list_sort(GtKmerPosList *kmerpos_list)
{
  ...
  if (kmerpos_list->encode_info != NULL)
  {
    if (kmerpos_list->encode_info->bytes_kmerpos <= sizeof (GtUword))
    {
#ifdef SKDEBUG
      GtUword idx;
      GtCodetype previous_code = 0;
#endif

      gt_radixsort_inplace_ulong_generic(kmerpos_list->radixsortinfo_ulong,
                                         kmerpos_list->spaceGtUword,
                                         kmerpos_list->nextfree);
#ifdef SKDEBUG
      ...
      {
        GtCodetype code
          = gt_kmerpos_ulong_code(kmerpos_list->spaceGtUword[idx],
                                  kmerpos_list->encode_info->code_shift);
        gt_assert(previous_code <= code);
        previous_code = code;
      }
#endif
    } else
    {
      ...
      gt_radixsort_inplace_flba(kmerpos_list->bytestring,
                                kmerpos_list->nextfree,
                                kmerpos_list->encode_info->bytes_kmerpos,
                                kmerpos_list->encode_info->bytes_code);
#ifdef SKDEBUG
      for (idx = 0; idx < kmerpos_list->nextfree; idx++)
      {
        GtCodetype code
          = gt_kmerpos_bytestring_code(kmerpos_list->bytestring + offset,
                                       kmerpos_list->encode_info->code_shift);
        gt_assert(previous_code <= code);
        previous_code = code;
        offset += kmerpos_list->encode_info->bytes_kmerpos;
      }
#endif
    }
  } else
...
static void gt_kmerpos_list_show(FILE *stream,const GtKmerPosList *kmerpos_list)
{
  ...
  for (idx = 0; idx < kmerpos_list->nextfree; idx++)
  {
    ...
    if (kmerpos_list->encode_info != NULL)
    {
      gt_kmerpos_ulong_decode(&dec,kmerpos_list->spaceGtUword[idx],
...
static void gt_diagbandseed_processkmercode(void *vpkinfo,
                                            bool firstinrange,
                                            GtUword startpos,
                                            GtCodetype code)
{
  GtDiagbandseedProcKmerInfo *pkinfo = (GtDiagbandseedProcKmerInfo *) vpkinfo;
  GtDiagbandseedKmerPos kmerpos_entry;

  gt_assert(pkinfo != NULL);
  /*gt_assert(pkinfo->sample_distance == 5 || pkinfo->sample_distance == 6);*/
  ...
  if (kmerpos_entry.endpos % pkinfo->sample_distance == 0)
  {
                                         ...
                                         GtUword idx)
{
  ...
  } else
  {
    ...
    *code_bits = (GtBitcount_type) gt_required_bits(num_different_kmers);
  }
  *bits_seqnum = (GtBitcount_type) gt_required_bits(numofsequences);
  *bits_endpos = (GtBitcount_type) gt_required_bits(max_endpos);
}
                                       ...
                                       GtUword idx)
{
  ...
  gt_diagbandseed_kmerpos_bits(&code_bits,
                               ...
                               seqranges,
                               idx);
  bytes_kmerpos = gt_bits2bytes(code_bits + bits_seqnum + bits_endpos +
                                score_bits);
  kmplt = gt_diagbandseed_base_type(bytes_kmerpos,kmplt,false);
      ...
      kmplt == GT_DIAGBANDSEED_BASE_LIST_BYTESTRING)
  {
    ...
    encode_info->score_bits = score_bits;
    encode_info->score_threshold = score_threshold;
    encode_info->bytes_kmerpos = bytes_kmerpos;
    encode_info->bytes_code = gt_bits2bytes(code_bits);
    ...
    encode_info->mask_seqnum = (((GtUword) 1) << bits_seqnum) - 1;
    encode_info->mask_endpos = (((GtUword) 1) << bits_endpos) - 1;
    encode_info->mask_score_bits = (((GtUword) 1) << score_bits) - 1;
    ...
    encode_info->bits_left_adjust_score
      = encode_info->b_endpos_shift - score_bits;
...
static uint32_t gt_diagbandseed_ulong_longest_code_run(
                         ...
                         GtUword length,
                         int code_shift,
                         bool with_distribution)
{
  GtUword idx, longest_code_run = 1, current_code_run = 1;
  GtCodetype previouscode;
  GtDiscDistri *code_run_dist = NULL;

  gt_assert(spaceGtUword != NULL && length > 0);
  previouscode = gt_kmerpos_ulong_code(spaceGtUword[0],code_shift);
  (*differentcodes) = 1;
  if (with_distribution)
  {
    code_run_dist = gt_disc_distri_new();
  }
  for (idx = 1; idx < length; idx++)
  {
    const GtCodetype currentcode
      = gt_kmerpos_ulong_code(spaceGtUword[idx],code_shift);
    ...
    {
      if (code_run_dist != NULL)
      {
        gt_disc_distri_add(code_run_dist, current_code_run);
      }
  ...
  {
    gt_disc_distri_add(code_run_dist, current_code_run);
    gt_disc_distri_show(code_run_dist,NULL);
    gt_disc_distri_delete(code_run_dist);
  }
  if (current_code_run > longest_code_run)
  {
    longest_code_run = current_code_run;
...
static uint32_t gt_diagbandseed_bytestring_longest_code_run(
                         GtUword *differentcodes,
                         const uint8_t *bytestring,
                         GtUword length,
                         size_t bytes_kmerpos,
                         ...
                         bool with_distribution)
{
  GtUword idx, last_idx = length * bytes_kmerpos,
          previouscode, longest_code_run = 1, current_code_run = 1;
  GtDiscDistri *code_run_dist = NULL;

  gt_assert(bytestring != NULL && bytes_kmerpos > sizeof (GtUword));
  previouscode = gt_kmerpos_bytestring_code(bytestring,code_shift);
  (*differentcodes) = 1;
  if (with_distribution)
  {
    code_run_dist = gt_disc_distri_new();
  }
  for (idx = (GtUword) bytes_kmerpos; idx < last_idx; idx += bytes_kmerpos)
  {
    const GtCodetype currentcode
      = gt_kmerpos_bytestring_code(bytestring + idx,code_shift);
    if (previouscode == currentcode)
    {
      current_code_run++;
    } else
...
static uint32_t gt_diagbandseed_longest_code_run(GtUword *differentcodes,
                                                 const GtKmerPosList
                                                   *kmerpos_list,
                                                 bool with_code_run_dist)
{
  ...
  if (kmerpos_list->encode_info != NULL)
  {
    if (kmerpos_list->encode_info->bytes_kmerpos <= sizeof (GtUword))
    {
      return gt_diagbandseed_ulong_longest_code_run(differentcodes,
                                                    kmerpos_list->spaceGtUword,
                                                    kmerpos_list->nextfree,
                                                    kmerpos_list->encode_info
                                                                ->code_shift,
    ...
    {
      return gt_diagbandseed_bytestring_longest_code_run(
                                                    differentcodes,
                                                    kmerpos_list->bytestring,
                                                    kmerpos_list->nextfree,
                                                    kmerpos_list->encode_info
                                   ...
                                   FILE *stream)
{
  ...
  if (verbose)
  {
    fprintf(stream, "# start fetching %u-mers for sequence range "
                    GT_WU "..." GT_WU,seedlength,seqrange_start,seqrange_end);
    if (encode_info != NULL)
    {
  ...
  pkinfo.sample_distance = sample_distance;
  ...
  if (kmerpos_list->nextfree > 0)
  {
    ...
    kmerpos_list->longest_code_run
      = gt_diagbandseed_longest_code_run(&kmerpos_list->differentcodes,
                                         kmerpos_list,
                                         with_code_run_dist);
  } else
  {
...
typedef struct {
  ...
  bool at_list_end;
  uint8_t *kenv_score_tab;
  GtUword kenv_score_tab_length;
  GtBitcount_type score_bits;
  GtUword mask_score_bits;
  /* for list based iterator */
  const GtKmerPosList *original;
  const GtDiagbandseedKmerPos *listend_struct;
  GtDiagbandseedKmerPos *listptr_struct, *listptr_struct_ref;
...
static void gt_diagbandseed_kmer_iter_reset(GtDiagbandseedKmerIterator *kmi)
{
  gt_assert(kmi != NULL);
  kmi->at_list_end = false;
  if (kmi->original != NULL) /* list based */
  {
    if (kmi->section.encode_info != NULL)
    {
      if (kmi->section.encode_info->bytes_kmerpos <= sizeof (GtUword))
      {
        kmi->listptr_uword = kmi->original->spaceGtUword;
        kmi->section.spaceGtUword = kmi->original->spaceGtUword;
      } else
      {
        kmi->listptr_uint8_t = kmi->original->bytestring;
        kmi->section.bytestring = kmi->original->bytestring;
      }
      kmi->listptr_struct_ref = kmi->listptr_struct
                              = kmi->section.spaceGtDiagbandseedKmerPos;
    } else
    {
      kmi->listptr_struct_ref = kmi->listptr_struct
                              = kmi->original->spaceGtDiagbandseedKmerPos;
      kmi->section.spaceGtDiagbandseedKmerPos
        = kmi->original->spaceGtDiagbandseedKmerPos;
    }
    if (gt_kmerpos_list_num_entries(kmi->original) == 0)
    {
      kmi->at_list_end = true;
    }
  } else /* file based */
  {
    if (kmi->section.encode_info != NULL)
    {
      if (kmi->section.encode_info->bytes_kmerpos <= sizeof (GtUword))
      {
        kmi->kmerstream_uword.nextread = kmi->kmerstream_uword.nextfree = 0;
        fseek(kmi->kmerstream_uword.fp,sizeof (GtKmerPosListHeaderInfo),
              SEEK_SET);
        if (gt_readnextfromstream_GtUword(&kmi->buffer_uword,
                                          &kmi->kmerstream_uword) != 1)
        {
          kmi->at_list_end = true;
        }
      } else
      {
        kmi->kmerstream_uint8_t.nextread = kmi->kmerstream_uint8_t.nextfree
                                         = 0;
        fseek(kmi->kmerstream_uint8_t.fp,sizeof (GtKmerPosListHeaderInfo),
              SEEK_SET);
        kmi->buffer_uint8_t
          = gt_readnextfromstream_uint8_t(&kmi->kmerstream_uint8_t);
        if (kmi->buffer_uint8_t == NULL)
        {
          kmi->at_list_end = true;
        }
      }
    ...
    {
      kmi->kmerstream_struct.nextread = kmi->kmerstream_struct.nextfree = 0;
      fseek(kmi->kmerstream_struct.fp,sizeof (GtKmerPosListHeaderInfo),
            SEEK_SET);
      if (gt_readnextfromstream_GtDiagbandseedKmerPos(kmi->listptr_struct,
                                                      &kmi->kmerstream_struct)
                                                      != 1)
      {
        kmi->at_list_end = true;
      }
...
static GtDiagbandseedKmerIterator *gt_diagbandseed_kmer_iter_new_list(
                               const GtKmerPosList *original)
{
  GtDiagbandseedKmerIterator *kmi = gt_malloc(sizeof *kmi);

  gt_assert(original != NULL);
  kmi->original = original;
  kmi->kenv_score_tab = NULL;
  kmi->kenv_score_tab_length = 0;
  kmi->score_bits = 0;
  kmi->mask_score_bits = 0;
  if (original->encode_info != NULL)
  {
    if (original->encode_info->score_bits > 0)
    {
      kmi->kenv_score_tab_length = original->longest_code_run;
      kmi->kenv_score_tab = gt_malloc(sizeof *kmi->kenv_score_tab *
                                      kmi->kenv_score_tab_length);
      kmi->score_bits = original->encode_info->score_bits;
      kmi->mask_score_bits = (((GtUword) 1) << kmi->score_bits) - 1;
    }
    if (original->encode_info->bytes_kmerpos <= sizeof (GtUword))
    {
      gt_assert(original->spaceGtUword != NULL);
      kmi->listend_uword = original->spaceGtUword + original->nextfree;
    } else
    {
      gt_assert(original->bytestring != NULL);
      kmi->listend_uint8_t = original->bytestring +
                             original->nextfree *
                             original->encode_info->bytes_kmerpos;
    }
    kmi->section.spaceGtDiagbandseedKmerPos
      = gt_malloc(sizeof *kmi->section.spaceGtDiagbandseedKmerPos
                  * original->longest_code_run);
    kmi->listend_struct = kmi->section.spaceGtDiagbandseedKmerPos +
                          original->longest_code_run;
  } else
  {
    gt_assert(original->spaceGtDiagbandseedKmerPos != NULL);
    kmi->listend_uword = NULL;
    kmi->listend_uint8_t = NULL;
    kmi->listend_struct = original->spaceGtDiagbandseedKmerPos +
                          original->nextfree;
  }
  kmi->section.encode_info = original->encode_info;
  gt_diagbandseed_kmer_iter_reset(kmi);
  return kmi;
}
...
{
  GtDiagbandseedKmerIterator *kmi = gt_malloc(sizeof *kmi);
  GtKmerPosListHeaderInfo kmerpos_list_header_info;

  kmi->original = NULL;
  kmi->score_bits = 0;
  kmi->mask_score_bits = 0;
  kmi->listend_uword = kmi->listptr_uword = NULL;
  kmi->listend_uint8_t = kmi->listptr_uint8_t = NULL;
  kmi->kenv_score_tab = NULL;
  kmi->kenv_score_tab_length = 0;
  gt_assert(fp != NULL);
  gt_xfread(&kmerpos_list_header_info,sizeof kmerpos_list_header_info,1,fp);
  if (encode_info != NULL)
  {
    if (encode_info->score_bits > 0)
    {
      kmi->kenv_score_tab_length = kmerpos_list_header_info.longest_code_run;
      kmi->kenv_score_tab = gt_malloc(sizeof *kmi->kenv_score_tab *
                                      kmi->kenv_score_tab_length);
      kmi->score_bits = encode_info->score_bits;
      kmi->mask_score_bits = (((GtUword) 1) << kmi->score_bits) - 1;
    }
    kmi->section.spaceGtUword = NULL;
    kmi->section.bytestring = NULL;
    kmi->section.allocated = kmerpos_list_header_info.longest_code_run;
    if (encode_info->bytes_kmerpos <= sizeof (GtUword))
    {
      kmi->kmerstream_uword.fp = fp;
      GT_BUFFEREDFILE_BUFFER_SET(&kmi->kmerstream_uword,
                                 file_buffer_size,
                                 sizeof *kmi->kmerstream_uword.
                                        bufferedfilespace);
    } else
    {
      kmi->kmerstream_uint8_t.fp = fp;
      GT_BUFFEREDFILE_BUFFER_SET(&kmi->kmerstream_uint8_t,
                                 file_buffer_size,
                                 encode_info->bytes_kmerpos *
                                 sizeof *kmi->kmerstream_uint8_t.
                                        bufferedfilespace);
    }
  } else
  {
    kmi->kmerstream_struct.fp = fp;
    GT_BUFFEREDFILE_BUFFER_SET(&kmi->kmerstream_struct,
                               file_buffer_size,
                               sizeof *kmi->kmerstream_struct.bufferedfilespace
                               );
    kmi->section.allocated
      = kmerpos_list_header_info.longest_code_run + 1;
  ...
  kmi->listend_struct = kmi->section.spaceGtDiagbandseedKmerPos +
                        kmi->section.allocated;
  kmi->section.encode_info = encode_info;
  gt_diagbandseed_kmer_iter_reset(kmi);
  return kmi;
}
                                             ...
                                             *encode_info)
{
  ...
  if (encode_info != NULL)
  {
    if (encode_info->score_threshold > 0)
    {
      gt_str_append_char(str, '-');
      gt_str_append_uint(str, encode_info->score_threshold);
    }
    if (encode_info->bytes_kmerpos <= sizeof (GtUword))
    {
                                               ...
                                               size_t file_buffer_size)
{
  ...
  if (list_fp != NULL)
  {
    ...
    kmeriterator = gt_diagbandseed_kmer_iter_new_from_fp(list_fp,
                                                         encode_info,
                                                         file_buffer_size);
  }
  gt_free(list_file);
...
static void gt_diagbandseed_kmer_iter_delete(GtDiagbandseedKmerIterator *kmi)
{
  if (kmi != NULL)
  {
    if (kmi->original == NULL)
    { /* file based */
      if (kmi->section.encode_info != NULL)
      {
        if (kmi->section.encode_info->bytes_kmerpos <= sizeof (GtUword))
        {
          gt_free(kmi->kmerstream_uword.bufferedfilespace);
          fclose(kmi->kmerstream_uword.fp);
          gt_free(kmi->section.spaceGtUword);
        } else
        {
          gt_free(kmi->kmerstream_uint8_t.bufferedfilespace);
          fclose(kmi->kmerstream_uint8_t.fp);
          gt_free(kmi->section.bytestring);
        }
      } else
      {
        gt_free(kmi->kmerstream_struct.bufferedfilespace);
        fclose(kmi->kmerstream_struct.fp);
      }
      gt_free(kmi->section.spaceGtDiagbandseedKmerPos);
      kmi->section.allocated = 0;
    } else
    {
      if (kmi->section.encode_info != NULL)
      {
                                              ...
                                              GtDiagbandseedKmerIterator *kmi)
{
  ...
  { /* list based */
    if (kmi->section.encode_info != NULL)
    {
      ...
      if (kmi->section.encode_info->bytes_kmerpos <= sizeof (GtUword))
      {
        code = gt_kmerpos_ulong_code(*kmi->listptr_uword,
        ...
        } while (kmi->listptr_uword < kmi->listend_uword &&
                 code == gt_kmerpos_ulong_code(*kmi->listptr_uword,
                                               kmi->section.encode_info
                                                           ->code_shift));
        if (kmi->listptr_uword >= kmi->listend_uword)
        {
          kmi->at_list_end = true;
        }
      } else
        ...
        do
        {
          ...
          kmi->listptr_uint8_t += kmi->section.encode_info->bytes_kmerpos;
                 ...
                 code == gt_kmerpos_bytestring_code(kmi->listptr_uint8_t,
                                                    kmi->section.encode_info
                                                                ->code_shift));
        if (kmi->listptr_uint8_t >= kmi->listend_uint8_t)
        {
          kmi->at_list_end = true;
        }
      }
    ...
    {
      code = kmi->listptr_struct->code;
      kmi->section.spaceGtDiagbandseedKmerPos = kmi->listptr_struct;
      /* add element to section until code differs */
      do
      {
        kmi->listptr_struct++;
      } while (kmi->listptr_struct < kmi->listend_struct &&
               code == kmi->listptr_struct->code);
      if (kmi->listptr_struct >= kmi->listend_struct)
      {
        kmi->at_list_end = true;
      }
    }
  ...
  { /* file based */
    ...
    if (kmi->section.encode_info != NULL)
    {
      ...
      {
        /* kmplt == ULONG */
        code = gt_kmerpos_ulong_code(kmi->buffer_uword,
                                     kmi->section.encode_info->code_shift);
        do
        {
          gt_assert(kmi->listptr_struct < kmi->listend_struct);
          ...
          gt_kmerpos_ulong_decode(kmi->listptr_struct++,
                                  kmi->buffer_uword,
                                  kmi->section.encode_info);
          rval = gt_readnextfromstream_GtUword(&kmi->buffer_uword,
                                               &kmi->kmerstream_uword);
        } while (rval == 1 &&
                 code == gt_kmerpos_ulong_code(kmi->buffer_uword,
                                               kmi->section.encode_info
                                                           ->code_shift));
      ...
      {
        /* kmplt == BYTESTRING */
        code = gt_kmerpos_bytestring_code(&kmi->buffer_uint8_t[0],
                                          kmi->section.encode_info
                                                      ->code_shift);
        do
        {
    ...
    {
      /* kmplt == STRUCT */
      if (kmi->section.spaceGtDiagbandseedKmerPos < kmi->listptr_struct)
      {
        kmi->section.spaceGtDiagbandseedKmerPos[0] = *kmi->listptr_struct;
        kmi->listptr_struct = kmi->section.spaceGtDiagbandseedKmerPos;
      }
      code = kmi->listptr_struct->code;
      do
      {
        kmi->listptr_struct++;
        gt_assert(kmi->listptr_struct < kmi->listend_struct);
        rval = gt_readnextfromstream_GtDiagbandseedKmerPos(
                                                       kmi->listptr_struct,
                                                       &kmi->kmerstream_struct);
      } while (rval == 1 && code == kmi->listptr_struct->code);
    }
    if (rval != 1)
    {
      kmi->at_list_end = true;
  ...
  kmi->section.nextfree = (GtUword) (kmi->listptr_struct -
                                    kmi->section.spaceGtDiagbandseedKmerPos);
  return &kmi->section;
}
...
{
  return "specify type of kmerpos, possible values are struct, bytestring, "
         "and ulong";
}
...
typedef struct
{
  ...
  GtDiagbandseedBaseListType splt;
  GtUword mask_tab[4], spl_transfer_mask,
  ...
  GtBitcount_type bits_seedpair,
                  ...
                  bits_units[2],
                  bits_left_adjust[4],
                  bits_left_adjust_score;
  size_t bytes_seedpair,
         bytes_seedpair_sort_key;
  int shift_tab[4],
      transfer_shift,
      bits_unused_in2GtUword,
      maxmat_format,
      score_threshold;
  GtUword amaxlen;
  bool maxmat_diagonalchain,
       inseqseeds;
                        ...
                        GtUword amaxlen)
{
  ...
  gt_assert(maxmat <= 4);
  seedpairlist->maxmat_format = gt_diagbandseed_derive_maxmat_format(maxmat);
  seedpairlist->maxmat_diagonalchain = maxmat > 0 && maxmat <= 3 ? true : false;
  seedpairlist->amaxlen = amaxlen;
  seedpairlist->inseqseeds = inseqseeds;
  seedpairlist->score_bits = score_bits;
  seedpairlist->mask_score_bits = (((GtUword) 1) << score_bits) - 1;
  seedpairlist->score_threshold = score_threshold;
  ...
  seedpairlist->bits_values[idx_aseqnum]
    = (GtBitcount_type) gt_required_bits(anumofseq);
  seedpairlist->bits_values[idx_bseqnum]
    = (GtBitcount_type) gt_required_bits(bnumofseq);
  if (seedpairlist->maxmat_diagonalchain)
  {
    seedpairlist->bits_values[idx_bpos]
      = (GtBitcount_type) gt_required_bits(
                             seedpairlist->aseqrange_max_length +
  ...
  {
    seedpairlist->bits_values[idx_bpos]
      = (GtBitcount_type) gt_required_bits(seedpairlist->bseqrange_max_length);
  }
  seedpairlist->bits_values[idx_apos]
    = (GtBitcount_type) gt_required_bits(seedpairlist->aseqrange_max_length);
  ...
  gt_assert(seedpairlist->bits_units[1] + score_bits <= gt_uword_bits);
  seedpairlist->bits_left_adjust_score
    = gt_uword_bits - (seedpairlist->bits_units[1] + score_bits);
  seedpairlist->spl_transfer_mask
  ...
  seedpairlist->bits_seedpair += score_bits;
  ...
  seedpairlist->bits_unused_in2GtUword = 2 * gt_uword_bits -
                                         seedpairlist->bits_seedpair;
  seedpairlist->bytes_seedpair = gt_bits2bytes(seedpairlist->bits_seedpair);
  /* only use for bytestring */
  if (fullseedpairsort)
  {
    seedpairlist->bytes_seedpair_sort_key = seedpairlist->bytes_seedpair;
  } else
  {
    const GtBitcount_type bits_seedpair_sort_key
      = seedpairlist->bits_seedpair -
        (seedpairlist->bits_values[idx_apos] + score_bits);
    seedpairlist->bytes_seedpair_sort_key
      = gt_bits2bytes(bits_seedpair_sort_key);
  }
  ...
  seedpairlist->bitbuffer = gt_bitbuffer_new();
  return seedpairlist;
}
                                      ...
                                      const GtSeedpairlist *seedpairlist)
{
  ...
  fprintf(stream,"apos=%hu bits",seedpairlist->bits_values[idx_apos]);
  if (seedpairlist->score_bits > 0)
  {
    fprintf(stream,", score=%hu bits\n",seedpairlist->score_bits);
  } else
  {
    fprintf(stream,"\n");
  }
...
static void gt_seedpairlist_delete(GtSeedpairlist *seedpairlist)
{
  if (seedpairlist != NULL)
  {
    ...
    gt_bitbuffer_delete(seedpairlist->bitbuffer);
...
#ifdef SKDEBUG
static uint8_t gt_diagbandseed_decode_seedpair_gen(
                                            GtDiagbandseedSeedPair *seedpair,
                                            const GtSeedpairlist *seedpairlist,
                                            const uint8_t *bytestring);

static char *gt_uint8_bits(const uint8_t byte)
{
  int idx;
  char *out = malloc(sizeof *out * (CHAR_BIT+1));
  uint8_t mask = ((uint8_t) 1) << 7;

  out[CHAR_BIT] = '\0';
  for (idx = 0; idx < CHAR_BIT; idx++)
  {
    out[idx] = (byte & mask) ? '1' : '0';
    mask >>= 1;
  }
  return out;
}

static void gt_out_bytestring(const uint8_t *bytestring,GtUword length)
{
  GtUword idx;

  for (idx = 0; idx < length; idx++)
  {
    char *s = gt_uint8_bits(bytestring[idx]);
    printf("%s ",s);
    free(s);
  }
  printf("\n");
...
static void gt_diagbandseed_encode_seedpair_bf(uint8_t *bytestring,
                                            const GtSeedpairlist *seedpairlist,
                                            ...
                                            uint8_t bpos_score_diff)
{
  GtUword bytestring_offset
    = gt_bitbuffer_write_bytestring(seedpairlist->bitbuffer,
                                    bytestring,
                                    0,
                                    seedpairlist->bytes_seedpair,
                                    (GtUword) aseqnum,
  ...
  gt_bitbuffer_bytestring_flush(false,seedpairlist->bitbuffer,
                                            ...
                                            uint8_t bpos_score_diff)
{
  ...
  value_positions
    |= (((GtUword) bpos_score_diff) << seedpairlist->bits_left_adjust_score);
...
#ifdef SKDEBUG
  {
    ...
    gt_assert(thisseedpair.bpos == bpos);
    gt_assert(thisbpos_score_diff == bpos_score_diff);
  }
#endif
}

#ifdef SKDEBUG
static void gt_diagbandseed_encode_seedpair_verify(const GtSeedpairlist
                                                      *seedpairlist)
{
  int idx;
  ...
  gt_assert(seedpairlist->bytes_seedpair <= bytestring_length);
  for (idx = 0; idx < 4; idx++)
  {
    max_values[idx] = (((GtUword) 1) << seedpairlist->bits_values[idx]) - 1;
  }
  max_values[4] = (((GtUword) 1) << seedpairlist->score_bits) - 1;
  for (trials = 0; trials < 1000000; trials++)
  {
    GtDiagbandseedSeqnum
      aseqnum = (GtDiagbandseedSeqnum) gt_rand_max(max_values[idx_aseqnum]),
      bseqnum = (GtDiagbandseedSeqnum) gt_rand_max(max_values[idx_bseqnum]);
    GtDiagbandseedPosition
      bpos = (GtDiagbandseedPosition) gt_rand_max(max_values[idx_bpos]),
      apos = (GtDiagbandseedPosition) gt_rand_max(max_values[idx_apos]);
    uint8_t bpos_score_diff = 0;
    if (seedpairlist->score_bits > 0)
    {
      bpos_score_diff = (uint8_t) gt_rand_max(max_values[4]);
    }
    gt_diagbandseed_encode_seedpair(bytestring,
                                    ...
                                    apos,
                                    bpos_score_diff);
    gt_diagbandseed_encode_seedpair_bf(bytestring_bf,
                                       seedpairlist,
                                       aseqnum,
                                       bseqnum,
                                       bpos,
                                       apos,
                                       bpos_score_diff);
    if (memcmp(bytestring,bytestring_bf,seedpairlist->bytes_seedpair) != 0)
    {
      gt_out_bytestring(bytestring,seedpairlist->bytes_seedpair);
      gt_out_bytestring(bytestring_bf,seedpairlist->bytes_seedpair);
      exit(EXIT_FAILURE);
    }
    GtDiagbandseedSeedPair thisseedpair;
    uint8_t thisbpos_score_diff
                                            ...
                                            const uint8_t *bytestring)
{
  ...
  transfer = value_seqnums & seedpairlist->spl_transfer_mask;
  value_positions
    = gt_diagbandseed_bytestring2GtUword(bytestring + sizeof (GtUword),
                                         seedpairlist->bytes_seedpair -
                                         sizeof (GtUword));
  value_positions >>= seedpairlist->bits_unused_in2GtUword;
  value_positions |= (transfer << seedpairlist->transfer_shift);
  bpos_score_diff = (uint8_t) (value_positions & seedpairlist->mask_score_bits);
  value_positions >>= seedpairlist->score_bits;
...
{
  uint8_t bpos_score_diff
    = gt_diagbandseed_decode_seedpair_gen(seedpair,
                                          seedpairlist,
                                          seedpairlist->mlist_bytestring->
                                                        spaceuint8_t + offset);
                                      ...
                                      GtUword spidx)
{
  ...
  } else
  {
    ...
    } else
    {
      ...
      seedpair->aseqnum += seedpairlist->aseqrange_start;
      seedpair->bseqnum += seedpairlist->bseqrange_start;
    }
    return bpos_score;
  }
}
                                      ...
                                      GtUword spidx)
{
  ...
  if (seedpairlist->score_bits > 0)
  {
    fprintf(stream,"," GT_KENV_SCORE_FORMAT_CHAR ")",bpos_score);
  } else
  {
    fprintf(stream,")");
  }
                                         ...
                                         const GtUword current)
{
  ...
  c_bpos_score = gt_seedpairlist_at(&c_seedpair,seedpairlist,current);
  ...
  if (seedpairlist->bytes_seedpair == seedpairlist->bytes_seedpair_sort_key)
  {
    if (p_apos < c_apos)
    {
      return -1;
                                  ...
                                  bool inseqseeds)
{
  ...
  while (alist != NULL && blist != NULL)
  {
    ...
    const GtUword alen = alist->nextfree,
                  blen = blist->nextfree;
      ...
      } else
      {
        ...
        if (frequency <= maxfreq)
        {
          ...
          } else
          {
            ...
            gt_assert(biter->kenv_score_tab == NULL ||
                      blen <= biter->kenv_score_tab_length);
            for (aptr = asegment; aptr < asegment + alen; aptr++)
            {
              for (bptr = bsegment; bptr < bsegment + blen; bptr++)
              {
                     ...
                     aptr->endpos + seedpairdistance->end >= bptr->endpos))
                {
                  ...
                  {
                    uint8_t bpos_score_diff
                      = biter->kenv_score_tab == NULL
                          ? 0
                          : biter->kenv_score_tab[(GtUword) (bptr - bsegment)];
                                  ...
                                  GtError *err)
{
  ...
  GtTimer *timer = NULL;
  GtUword idx;
  GtUchar *a_encoded = NULL, *b_encoded = NULL;
  char *a_decoded = gt_malloc(3 * (seedlength + 1) * sizeof *a_decoded),
       *b_decoded = a_decoded + 1 + seedlength,
       *b_decoded_rev = b_decoded + 1 + seedlength;

  a_decoded[seedlength] = b_decoded[seedlength]
                        = b_decoded_rev[seedlength] = '\0';
  if (verbose)
  {
  ...
  {
    a_encoded = gt_malloc(2 * (seedlength + 1) * sizeof *a_encoded);
    b_encoded = a_encoded + 1 + seedlength;
  }
  for (idx = 0; idx < mlistlen; idx++)
  {
    ...
    gt_encseq_extract_decoded(aencseq, a_decoded, abs_apos + 1 - seedlength,
                              abs_apos);
    if (a_encoded != NULL)
    {
      gt_encseq_extract_encoded(aencseq, a_encoded, abs_apos + 1 - seedlength,
                                abs_apos);
    }
    ...
    if (!reverse)
    {
      ...
      gt_encseq_extract_decoded(bencseq, b_decoded, abs_bpos + 1 - seedlength,
                                abs_bpos);

      if (b_encoded != NULL)
      {
        gt_encseq_extract_encoded(bencseq, b_encoded, abs_bpos + 1 - seedlength,
                                  abs_bpos);
      }
      if (kenv_generator != NULL)
      {
        ...
        if (real_bpos_score != bpos_score)
        {
          gt_error_set(err, "Wrong SeedPair (%" PRIu32
                            ...
                            "real score = %d != %d = score",
                     seedpair.aseqnum,
                     seedpair.bseqnum,
                     apos,
                     GT_DIAGBANDSEED_CONV_B(apos,bpos),
                     a_decoded, b_decoded,
                     real_bpos_score,bpos_score);
          gt_free(a_encoded);
          gt_free(a_decoded);
          if (verbose)
          {
      ...
      {
        if (strcmp(a_decoded, b_decoded) != 0)
        {
          gt_error_set(err, "Wrong SeedPair (%" PRIu32
                            ",%" PRIu32 ",%" PRIu32 ",%" GT_WUS "): %s != %s\n",
    ...
    } else
    {
      ...
      gt_encseq_extract_decoded(bencseq, b_decoded, abs_bpos,
                                abs_bpos + seedlength - 1);

      for (bufptr = b_decoded_rev; bufptr < b_decoded_rev + seedlength;
           bufptr++)
      {
        gt_complement(bufptr,
                      b_decoded[seedlength + b_decoded_rev - bufptr - 1], NULL);
      }
      if (strcmp(a_decoded, b_decoded_rev) != 0)
      {
        gt_error_set(err, "Wrong SeedPair (" "%"PRIu32
                          ",%"PRIu32",%"PRIu32",%"PRIu32"): %s != %s\n",
                     seedpair.aseqnum, seedpair.bseqnum,
                     GT_DIAGBANDSEED_GETPOS_A(&seedpair),
                     GT_DIAGBANDSEED_GETPOS_B(&seedpair), a_decoded,
                     b_decoded_rev);
        gt_free(a_decoded);
        if (verbose)
        {
  ...
  if (verbose)
  {
    fprintf(stream, "# ...successfully verified all " GT_WU " seeds ",mlistlen);
    gt_timer_show_formatted(timer, GT_DIAGBANDSEED_FMT, stream);
    gt_timer_delete(timer);
  }
  gt_free(a_encoded);
                                            ...
                                            GtError *err)
{
  ...
  gt_assert(sizeofunit > 0 && memlimit < GT_UWORD_MAX);
  if (verbose)
  {
             ...
             FILE *fpout)
{
  ...
  {
    if (GT_DIAGBANDSEED_SAME_DIAGONAL(previous,current) &&
        previous.apos + 1 == current->apos)
    {
      previous_matchlength++;
      previous.apos++;
    } else
    {
      ...
      } else
      {
        ...
        {
          if (GT_DIAGBANDSEED_SAME_DIAGONAL(previous,current) &&
              previous.apos + seedlength <= current->apos)
          {
            printf("A %u\n",previous.apos+1);
          }
        } else
...
static void gt_diagbandseed_segment_exact_extension(
             GT_UNUSED void *v_segment2matches_info,
             GT_UNUSED bool bpos_sorted,
             const GtEncseq *aencseq,
             GT_UNUSED const GtEncseq *bencseq,
             GT_UNUSED GtUword aseqnum,
             GT_UNUSED GtUword bseqnum,
             GT_UNUSED const GtDiagbandStruct *diagband_struct,
             GT_UNUSED const GtDiagbandseedMaximalmatch *memstore,
             ...
             GtUword segment_length)
{
  ...
  gt_assert(memstore == NULL && aencseq != NULL);
...
#define GT_DIAGBANDSEED_PROCESS_SEGMENT\
        ...
        {\
          ...
          {\
            if (process_run == 0 && seedpairlist->maxmat_diagonalchain)\
            {\
              gt_diagbandseed_segment2maxmatches(forward,\
                                                 seedpairlist->maxmat_format,\
                                                 memstore,\
                                                 currsegm_aseqnum,\
                                                 currsegm_bseqnum,\
                                                 seedlength,\
            ...
            if (!seedpairlist->maxmat_diagonalchain || memstore != NULL)\
            {\
              if (diagband_struct != NULL && \
                  !seedpairlist->maxmat_diagonalchain)\
              {\
                if (esi != NULL && esi->same_encseq && \
                    !seedpairlist->inseqseeds)\
                {\
          ...
          {\
            gt_querymatch_segment_buffer_select(esi->querymatch_segment_buffer,\
                                                esi->out_display_flag,\
                                                esi->accu_match_values_entry);\
          }\
        }
                 ...
                 GtUword total_extension_time_usec)
{
  ...
  if (mlistlen > 0)
  {
    printf("# ... processed " GT_WU " seeds %sin "
           GT_WD ".%06ld seconds.\n",
           mlistlen,
           maxmat_format > 0 ? "" : "(excluding extensions) ",
                                            ...
                                            *segment_reject_info)
{
  ...
  if (seedpairlist->maxmat_format > 0 && seedpairlist->maxmat_format != 4)
  {
    if (seedpairlist->maxmat_format == 1)
    {
      fprintf(stream,"# Fields: s.len, s.seqnum, s.start, strand, q.seqnum, "
                     "q.start\n");
    } else
    {
      fprintf(stream,"# Fields: s.start, q.start, s.len\n");
    }
  } else
  {
    ...
    if (gt_str_length(diagband_statistics_arg) == 0)
    {
      ...
      if (seedpairlist->maxmat_format == 4)
      {
        segment_proc_func = gt_diagbandseed_segment_exact_extension;
      } else
      {
        segment_proc_func = gt_diagbandseed_segment2matches;
      }
      segment_proc_info = esi;
      gt_assert(esi != NULL);/* && seedpairlist->maxmat_format == 4);*/
    } else
    {
  ...
  if (seedpairlist->splt == GT_DIAGBANDSEED_BASE_LIST_STRUCT)
  {
    ...
    /* we do not need to handle scores from k-environments, as they are
       splt = STRUCT cannot be used with scores */
  ...
  } else
  {
    ...
    } else
    {
      ...
      bpos_score = gt_diagbandseed_decode_seedpair(&nextsegment,seedpairlist,0);
                                     ...
                                     bool with_code_run_dist)
{
  ...
  if (verbose)
  {
    const GtKenvScore max_score_qgram
      = gt_kenv_generator_get_max_score_qgram(kenv_generator);

    gt_assert(max_score_qgram >= kmerpos_list->encode_info->score_threshold);
    score_dist_size = max_score_qgram -
                      kmerpos_list->encode_info->score_threshold + 1;
    score_dist = gt_calloc(score_dist_size,sizeof *score_dist);
  }
  while (alist != NULL)
  {
    ...
    if (this_kmer_env_size > 0)
    {
      ...
      const GtUword all_pairs = this_kmer_env_size * alist->nextfree;
      uint8_t *kmer_environ_ptr_uint8 = NULL;
      GtUword *kmer_environ_ptr_uword = NULL;
#ifndef NDEBUG
      uint8_t *kmer_environ_end_uint8 = NULL;
      GtUword *kmer_environ_end_uword = NULL;
#endif
      ...
      if (verbose)
      {
        ...
        for (raw_ptr = raw_list; raw_ptr < raw_list + this_kmer_env_size;
             raw_ptr++)
        {
          gt_assert(raw_ptr->score >=
                    kmerpos_list->encode_info->score_threshold &&
                    raw_ptr->score -
                    kmerpos_list->encode_info->score_threshold <
                      score_dist_size);
      ...
      if (kmerpos_list->nextfree + all_pairs >= kmerpos_list->allocated)
      {
        kmerpos_list->allocated += (all_pairs + 0.2 * kmerpos_list->allocated);
        if (kmerpos_list->encode_info->bytes_kmerpos <= sizeof (GtUword))
        {
          kmerpos_list->spaceGtUword
            = gt_realloc(kmerpos_list->spaceGtUword,
      ...
      if (kmerpos_list->encode_info->bytes_kmerpos <= sizeof (GtUword))
      {
        kmer_environ_ptr_uword = kmerpos_list->spaceGtUword +
                                 kmerpos_list->nextfree;
#ifndef NDEBUG
        kmer_environ_end_uword = kmerpos_list->spaceGtUword +
                                 kmerpos_list->allocated;
#endif
      } else
      {
        kmer_environ_ptr_uint8 = kmerpos_list->bytestring +
                                 kmerpos_list->nextfree *
                                 kmerpos_list->encode_info->bytes_kmerpos;
#ifndef NDEBUG
        kmer_environ_end_uint8 = kmerpos_list->bytestring +
                                 kmerpos_list->allocated *
                                 kmerpos_list->encode_info->bytes_kmerpos;
#endif
      }
          ...
          if (kmerpos_list->encode_info->bytes_kmerpos <= sizeof (GtUword))
          {
            GtUword this_enc = GT_KMERPOS_GTUWORD_ENCODE(raw_ptr->kmer_neighbor,
                               ...
                               (GtUword) (raw_ptr->score -
                                          encode_info->score_threshold);
#ifdef SKDEBUG
            GtDiagbandseedKmerPos dec;
            gt_kmerpos_ulong_decode(&dec,this_enc,encode_info);
            gt_assert(dec.code == raw_ptr->kmer_neighbor);
            gt_assert(dec.seqnum == aptr->seqnum);
            gt_assert((GtUword) dec.endpos == endpos);
            gt_assert(encode_info->score_threshold +
                      (this_enc & encode_info->mask_score_bits)
                      == (GtUword) raw_ptr->score);
#endif
            gt_assert(kmer_environ_ptr_uword < kmer_environ_end_uword);
            *kmer_environ_ptr_uword = this_enc;
            kmer_environ_ptr_uword++;
          } else
          {
            gt_assert(kmer_environ_ptr_uint8 < kmer_environ_end_uint8);
            gt_kmerpos_bytestring_encode(kmer_environ_ptr_uint8,
                                         encode_info,
                                         raw_ptr->kmer_neighbor,
                                         seqnum,
                                         endpos,
                                         encode_info->bits_left_adjust_score,
                                         (GtUword) (raw_ptr->score -
...
#ifdef SKDEBUG
            {
              GtDiagbandseedKmerPos dec;
              uint8_t bpos_score_diff
                = gt_kmerpos_bytestring_decode(&dec,kmer_environ_ptr_uint8,
                                               encode_info);
              gt_assert(dec.code == raw_ptr->kmer_neighbor);
              gt_assert(dec.seqnum == aptr->seqnum);
              gt_assert(dec.endpos == aptr->endpos);
              gt_assert(bpos_score_diff == (raw_ptr->score -
                                            encode_info->score_threshold));
            }
#endif
            kmer_environ_ptr_uint8 += encode_info->bytes_kmerpos;
          }
        }
      }
      kmerpos_list->nextfree += all_pairs;
    }
    alist = gt_diagbandseed_kmer_iter_next(kenv_src_iter);
  }
  if (verbose)
  ...
  {
    int idx;
    const size_t size_kmerpos
      = MAX(sizeof (GtUword),kmerpos_list->encode_info->bytes_kmerpos);

    for (idx = 0; idx < score_dist_size; idx++)
    {
      if (score_dist[idx] > 0)
      {
        fprintf(stream,"# score %d: " GT_WU " times\n",
                idx + kmerpos_list->encode_info->score_threshold,
                score_dist[idx]);
      }
    }
    fprintf(stream,"# (%d,%u)-environment has " GT_WU " elements (%.0f MB, "
           "%.2f per kmer in sequence)\n",
           gt_kenv_generator_get_q(kenv_generator),
           kmerpos_list->encode_info->score_threshold,
           kmerpos_list->nextfree,
           GT_MEGABYTES(kmerpos_list->nextfree * size_kmerpos),
           (double) kmerpos_list->nextfree/kmer_count);
  }
  gt_free(score_dist);
                                       ...
                                       GtError *err)
{
  ...
  if (verbose)
  {
    fprintf(stream,"# write " GT_WU " %u-mers with positions",
            kmerpos_list->nextfree,seedlength);
    if (kmerpos_list->encode_info != NULL &&
        kmerpos_list->encode_info->score_bits > 0)
    {
      fprintf(stream," and scores");
    }
    if (spacedseedweight < seedlength ||
        (kmerpos_list->encode_info != NULL &&
         kmerpos_list->encode_info->score_bits > 0))
    {
      fprintf(stream," (");
      if (spacedseedweight < seedlength)
      {
        fprintf(stream,"weight=%u",spacedseedweight);
      }
      if (kmerpos_list->encode_info != NULL &&
          kmerpos_list->encode_info->score_bits > 0)
      {
        ...
        fprintf(stream,"%hu=%hu+%hu+%hu+%hu bits=" GT_WU " bytes",
                kmerpos_list->encode_info->bits_kmerpos,
                kmerpos_list->encode_info->code_bits,
                kmerpos_list->encode_info->bits_seqnum,
                kmerpos_list->encode_info->bits_endpos,
                kmerpos_list->encode_info->score_bits,
                (GtUword) kmerpos_list->encode_info->bytes_kmerpos);
      }
      fprintf(stream,")");
    }
    fprintf(stream," to file %s\n",path);
    fprintf(stream,"# " GT_WU " different %u-mers, %.2f entries per %d-mer, ",
            kmerpos_list->differentcodes,seedlength,
            (double) kmerpos_list->nextfree/kmerpos_list->differentcodes,
            seedlength);
    fprintf(stream,"most frequent %d-mer occurs %u times\n",
                   seedlength,kmerpos_list->longest_code_run);
  }
  kmer_stream = gt_fa_fopen(path, "wb", err);
  if (kmer_stream == NULL)
  {
    return -1;
  }
  kmerpos_list_header_info.longest_code_run = kmerpos_list->longest_code_run;
  gt_assert(kmerpos_list->numofchars <= UINT8_MAX);
  kmerpos_list_header_info.numofchars = (uint8_t) kmerpos_list->numofchars;
  gt_assert(spacedseedweight <= UINT8_MAX);
  kmerpos_list_header_info.spacedseedweight = (uint8_t) spacedseedweight;
  if (kmerpos_list->encode_info != NULL)
  {
    kmerpos_list_header_info.bytes_kmerpos
      = kmerpos_list->encode_info->score_bits;
    if (kmerpos_list->encode_info->bytes_kmerpos <= sizeof (GtUword))
    {
      kmerpos_list_header_info.kmplt = GT_DIAGBANDSEED_BASE_LIST_ULONG;
    } else
    {
      kmerpos_list_header_info.kmplt = GT_DIAGBANDSEED_BASE_LIST_BYTESTRING;
    }
  } else
  {
    kmerpos_list_header_info.kmplt = GT_DIAGBANDSEED_BASE_LIST_STRUCT;
    kmerpos_list_header_info.bytes_kmerpos = sizeof (GtDiagbandseedKmerPos);
  }
  gt_xfwrite(&kmerpos_list_header_info,sizeof kmerpos_list_header_info,1,
             kmer_stream);
  if (kmerpos_list->nextfree > 0)
  {
    if (kmerpos_list->encode_info != NULL)
    {
      if (kmerpos_list->encode_info->bytes_kmerpos <= sizeof (GtUword))
      {
        ...
        gt_xfwrite(kmerpos_list->spaceGtUword,
                   sizeof *kmerpos_list->spaceGtUword,
                   kmerpos_list->nextfree, kmer_stream);
      } else
      {
        ...
        gt_xfwrite(kmerpos_list->bytestring,
                   sizeof *kmerpos_list->bytestring,
                   kmerpos_list->nextfree *
                   kmerpos_list->encode_info->bytes_kmerpos, kmer_stream);
      }
    } else
    {
      gt_assert(kmerpos_list->spaceGtDiagbandseedKmerPos != NULL);
  ...
  gt_fa_fclose(kmer_stream);
  return 0;
}

static void gt_diagbandseed_encode_info_pair_delete(
...
static GtDiagbandseedKmerIterator *gt_diagbandseed_kenv_iterator(
                  const GtDiagbandseedInfo *arg,
                  GtKmerPosList **kenv_list_ptr,
                  GtDiagbandseedKmerIterator *aiter,
                  const GtSequencePartsInfo *aseqranges,
                  GtUword aidx,
                  const GtSequencePartsInfo *bseqranges,
                  ...
                  const GtKmerPosListEncodeInfo *kenv_bencode_info,
                  FILE *stream,
                  GtError *err)
{
  GtUword blist_len,
  ...
  if (biter == NULL)
  {
    ...
    *kenv_list_ptr = gt_diagbandseed_kenv_generate(bidx,
    ...
    if (arg->use_kmerfile)
    {
      had_err = gt_diagbandseed_write_kmers(*kenv_list_ptr,
                                            kenv_path,
                                            arg->spacedseedweight,
                                            arg->seedlength,
                                            arg->verbose,
      ...
      gt_kmerpos_list_delete(*kenv_list_ptr);
      *kenv_list_ptr = NULL;
    } else
    {
      blist_len = (*kenv_list_ptr)->nextfree;
      biter = gt_diagbandseed_kmer_iter_new_list(*kenv_list_ptr);
    }
...
static int gt_diagbandseed_algorithm(const GtDiagbandseedInfo *arg,
                                     ...
                                     GtError *err)
{
  /* all the following variables will first be used for creating the iterators
     to the kmerposlist and are deleted once the seedpairlist is created */
  GtDiagbandseedKmerIterator *aiter = NULL,
                             *biter = NULL;
  GtKmerPosList *blist = NULL,
                *kenv_list = NULL;
  GtUword alist_len = 0, blist_len = 0;
  bool both_strands, selfcomp, equalranges, alist_blist_identical;
  ...
  GtUword mlistlen, maxfreq, len_used = 0;
  ...
  {
    score_bits = gt_kenv_generator_get_score_bits(arg->kenv_generator);
  ...
  if (arg->aencseq != arg->bencseq || aseqranges != bseqranges || aidx != bidx)
  {
    bencode_info
      = gt_kmerpos_encode_info_new(arg->kmplt,
                                   gt_encseq_alphabetnumofchars(arg->bencseq),
                                   arg->spacedseedweight,
                                   bseqranges,
                                   0,
                                   0,
  ...
  } else
  {
    ...
    if (arg->kenv_generator != NULL)
    {
      kenv_bencode_info = kenv_aencode_info;
    }
  }
  ...
  if (alist == NULL)
  {
    aiter = gt_diagbandseed_kmer_iter_new_from_file(&alist_len,
                                                    arg->aencseq,
                                                    arg->spacedseedweight,
                                                    arg->seedlength,
                                                    true,
  ...
  {
    alist_len = alist->nextfree;
    aiter = gt_diagbandseed_kmer_iter_new_list(alist);
  }
  if (arg->kenv_generator != NULL)
  {
    biter = gt_diagbandseed_kenv_iterator(arg,
                                          &kenv_list,
                                          aiter,
                                          aseqranges,
                                          aidx,
    ...
    } else
    {
      ...
      if (biter == NULL)
      {
        const GtReadmode readmode_kmerscan = arg->nofwd ? GT_READMODE_COMPL
                                                        : GT_READMODE_FORWARD;
        const GtUword known_size = (selfcomp && equalranges) ? alist_len : 0;

        blist = gt_diagbandseed_get_kmers(
                                arg->bencseq,
                                arg->spacedseedweight,
                                arg->seedlength,
                                arg->b_sample_distance,
                                arg->spaced_seed_spec,
                                readmode_kmerscan,
                                gt_sequence_parts_info_start_get(bseqranges,
                                                                 bidx),
                                gt_sequence_parts_info_end_get(bseqranges,bidx),
                                bencode_info,
                                arg->debug_kmer,
  ...
  if (!had_err)
  {
    ...
    if (seedpairlist->maxmat_diagonalchain && seedpairlist->maxmat_format == 0)
    {
  ...
  maxfreq = arg->maxfreq;
  mlistlen = 0;
  if (!had_err && seedpairlist->maxmat_format == 0 &&
      arg->memlimit < GT_UWORD_MAX)
  {
  ...
  if (!had_err)
  {
    ...
    mlistlen = gt_seedpairlist_length(seedpairlist);
    if (arg->verify_seeds && mlistlen > 0)
    {
  ...
  if (arg->snd_pass)
  {
    snd_pass_radix_sort_info = gt_radixsort_new_ulong(0,sizeof (GtUword));
  }
    ...
    if (both_strands)
    {
      ...
      if (biter == NULL)
      {
        ...
        clist = gt_diagbandseed_get_kmers(
                              ...
                              blist_len,
                              stream);
        gt_assert(blist_len == clist->nextfree);
        biter = gt_diagbandseed_kmer_iter_new_list(clist);
      }

      gt_diagbandseed_kmer_iter_reset(aiter);
      if (seedpairlist->maxmat_format == 0 && arg->memlimit < GT_UWORD_MAX)
      {
  ...
  gt_diagbandseed_encode_info_pair_delete(kenv_aencode_info,kenv_bencode_info);
                        ...
                        GtError *err)
{
  ...
  for (aidx = 0; !had_err && aidx < anumseqranges; aidx++)
  {
    ...
    GtKmerPosListEncodeInfo *aencode_info, *kenv_aencode_info = NULL;
    ...
    gt_kmerpos_encode_info_delete(aencode_info);
    gt_kmerpos_encode_info_delete(kenv_aencode_info);
  }
#ifdef GT_THREADS_ENABLED
  ...
  if (arg->verbose)
  {
    const int maxmat_format = gt_diagbandseed_derive_maxmat_format(arg->maxmat);

    gt_assert(dbs_state != NULL);
    if (!onlykmers)
    {
      gt_diagbandseed_dbs_state_out(dbs_state,maxmat_format,arg->snd_pass);
#ifndef _WIN32
