...
#ifndef NDEBUG
#define GT_SHOWFRONTINFO(CODELINES)\
        if (showfrontinfo)\
        {\
          CODELINES;\
        }
#else
#define GT_SHOWFRONTINFO(CODELINES) /* Nothing */
#endif
                             ...
                             GtUword distance)
{
  ...
  {
    GtUword thisalignedlen = GT_MULT2(ptr->row) + GT_FRONT_DIAGONAL(ptr);
