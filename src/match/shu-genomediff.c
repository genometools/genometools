...
static void genomediff_calculate_genome_length(const GtEncseq *encseq,
                      ...
                      GtError *err)
{
  ...
  if (!had_err) {
    ...
    genomediff_calculate_genome_length(encseq, genome_length, unit_info);
  ...
  if (!had_err) {
    ...
    if (seq_gc_content != NULL) {
      ...
      for (genome_idx = 0;
           genome_idx < unit_info->num_of_genomes;
           genome_idx++) {
