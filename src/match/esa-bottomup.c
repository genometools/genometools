...
#define PUSHDFS(LCP,LB,RMOSTBD,PREVIOUSPTR)\
        ...
        stackspace[nextfreeItvinfo].rmostbd = RMOSTBD;\
        nextfreeItvinfo++

typedef struct
{
  unsigned long lcp, lb, rb, rmostbd;
  GtBUinfo *info;
} GtBUItvinfo;
...
static void GtQueuepair_enumleaves(GtQueuepair *queuepair,unsigned long endpos)
{
  if (!GT_SUFTABELEMISUNDEF(queuepair->leftelem))
  {
    if (queuepair->leftelem.index <= endpos)
    {
      printf("elem=%lu\n",queuepair->leftelem.suffix);
      GT_SUFTABELEMSETUNDEF(queuepair->leftelem);
    } else
    {
      return;
