...
#define GT_RANDOMCODES_ACCUM(BUF,CODE,RELPOS)\
        {\
          if (((RELPOS) > 0) &&\
...
static void gt_randomcodes_accum_kmerscan_range(
                                         ...
                                         GtCodeposbuffer *buf)
{
  ...
  rccode = gt_kmercode_complement(gt_kmercode_reverse(fcode,kmersize),
                                  maskright);
  GT_RANDOMCODES_ACCUM(buf,fcode,0UL);
  if (kmersize == minmatchlength)
  {
    GT_RANDOMCODES_ACCUM(buf,rccode,lastfrelpos);
  }
  ...
  for (frelpos = 1UL; frelpos <= lastfrelpos; frelpos++)
  {
    ...
    {
      GT_RANDOMCODES_ACCUM(buf,fcode,frelpos);
    }
    if (lastfrelpos - frelpos <= lastpossiblepos)
    {
      GT_RANDOMCODES_ACCUM(buf,rccode,lastfrelpos - frelpos);
    }
    if (shiftright > 0)
    {
      shiftright -= 2;
...
static void gt_randomcodes_accum_kmerscan_eqlen(
                                     const GtBitsequence *twobitencoding,
                                     ...
                                     GtCodeposbuffer *buf)
{
  ...
  if (equallength >= (unsigned long) kmersize)
  {
    ...
    {
      gt_randomcodes_accum_kmerscan_range(twobitencoding,
                                         kmersize,
                                         minmatchlength,
                                         startpos,
                                         equallength,
...
static void gt_randomcodes_accum_kmerscan(const GtEncseq *encseq,
                                         const GtBitsequence *twobitencoding,
                                         unsigned long totallength,
                                         unsigned long maxunitindex,
                                         unsigned int kmersize,
...
void gt_randomcodes_accum_runkmerscan(const GtEncseq *encseq,
                                     unsigned int kmersize,
                                     unsigned int minmatchlength,
                                     GtCodeposbuffer *buf)
{
  ...
  if (gt_encseq_accesstype_get(encseq) == GT_ACCESS_TYPE_EQUALLENGTH)
  {
    ...
    gt_randomcodes_accum_kmerscan_eqlen(twobitencoding,
                                       equallength,
                                       totallength,
                                       maxunitindex,
                                       kmersize,
  ...
  {
    gt_randomcodes_accum_kmerscan(encseq,
                                 twobitencoding,
                                 totallength,
                                 maxunitindex,
                                 kmersize,
