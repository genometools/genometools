...
/* Implements the <GtNodeStream> interface. A <GtStatStream> gathers statistics
   about the <GtGenomeNode>s it retrieves from its node source and passes them
   along unmodified.  */
typedef struct GtStatStream GtStatStream;

/* Create a <GtStatStream*> which gathers statistics about the <GtGenomeNode>s
   it retrieves from its <in_stream> and returns them unmodified.
   Besides the basic statistics, statistics about the following distributions
   can be gathered, if the corresponding argument equals <true>:
   <gene_length_distribution>, <gene_score_distribution>,
   <exon_length_distribution>, <exon_number_distribution>,
   <intron_length_distribution>, <cds_length_distribution>. If <used_sources>
   equals <true>, it is recorded which source tags have been encountered. */
