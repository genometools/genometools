...
/* Sets the values of <orf_rng.start>, <orf_rng.end> and <orf_frame> to the
   current reading position of <ci> if START/STOP AA is found. The frame in
   which the ORF is located is written to the position pointed to by
   <orf_frame>. This function returns one of three status codes:
   GT_ORF_ITERATOR_OK    : an ORF was detected successfully(START/STOP AA pair),
   GT_ORF_ITERATOR_END   : no ORF was detected because the end of the scan
                           region has been reached,
   GT_ORF_ITERATOR_ERROR : no ORF was detected because an error occurred during
