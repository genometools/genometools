...
static GtIntset *gt_n_r_encseq_fill_ssp_tab(const GtEncseq *orig_es)
{
  GtIntset *ssptab;
  GtUword max, num, idx;
  num = gt_encseq_num_of_sequences(orig_es);
  max = gt_encseq_seqstartpos(orig_es, num - 1);
  /* we store the internal separators, the end is explicit */
  ssptab = gt_intset_best_new(max, num - 1);
  for (idx = (GtUword) 1; idx < num; ++idx) {
    GtUword pos = gt_encseq_seqstartpos(orig_es, idx);
    gt_assert(pos != 0);
...
static GtNREncseq *gt_n_r_encseq_new_empty(const GtEncseq *orig_es)
{
  ...
  n_r_encseq->ssptab = gt_n_r_encseq_fill_ssp_tab(orig_es);
  return n_r_encseq;
}
...
void gt_n_r_encseq_print_info(const GtNREncseq *nrencseq)
{
  GtUword av_link_len = 0,
          av_uque_len = 0,
          i;
  gt_log_log("length,uniqueid");
  for (i = 0; gt_log_enabled() && i < nrencseq->ldb_nelems; i++) {
    av_link_len += nrencseq->links[i].len;
    gt_log_log("link: " GT_WU "," GT_WU, nrencseq->links[i].len,
        nrencseq->links[i].unique_id);
  }
  for (i = 0; i < nrencseq->udb_nelems; i++) {
    av_uque_len += nrencseq->uniques[i].len;
    gt_log_log("unique: " GT_WU, nrencseq->uniques[i].len);
  }
  printf(GT_WU " link entries\n",nrencseq->ldb_nelems);
  if (nrencseq->ldb_nelems > 0) {
    av_link_len = av_link_len / nrencseq->ldb_nelems;
    printf("average length of link fragments: " GT_WU "\n", av_link_len);
  }
  printf(GT_WU " unique entries\n", nrencseq->udb_nelems);
  if (nrencseq->udb_nelems > 0) {
    av_uque_len = av_uque_len / nrencseq->udb_nelems;
    printf("average length of unique fragments: " GT_WU "\n", av_uque_len);
  }
  if (nrencseq->unique_es != NULL)
    printf("total udb length:" GT_WU "\n",
           gt_encseq_total_length(nrencseq->unique_es));
  if (nrencseq->orig_es != NULL)
    printf("original db length:" GT_WU "\n",
           gt_encseq_total_length(nrencseq->orig_es));
}

GtUword gt_n_r_encseq_get_orig_length(GtNREncseq *nrencseq) {
  return nrencseq->orig_length;
}

GtUword gt_n_r_encseq_get_unique_length(GtNREncseq *nrencseq) {
  return gt_encseq_total_length(nrencseq->unique_es);
}
...
/*n_r_encseq_IO*/
typedef void (*NREncseqIOFunc)(void *ptr,
                               size_t size,
                               size_t nmemb,
                               FILE *stream);

/* function definition for xfwrite fitting the function pointer*/
static inline void gt_n_r_encseq_gt_xfwrite(void *ptr,
                                            size_t size,
                                            size_t nmemb,
                                            FILE *stream)
{
...
/* function definition for xfread fitting the function pointer*/
static inline void gt_n_r_encseq_gt_xfread(void *ptr,
                                           size_t size,
                                           size_t nmemb,
                                           FILE *stream)
{
...
static void gt_n_r_encseq_linkentry_io(GtNREncseqLink* link,
                                       FILE* fp,
                                       EditscriptIOFunc io_func)
{
  gt_assert (link != NULL);
  io_func(&link->orig_startpos, sizeof (link->orig_startpos), (size_t) 1, fp);
  io_func(&link->len, sizeof (link->len), (size_t) 1, fp);
  io_func(&link->unique_id, sizeof (link->unique_id), (size_t) 1, fp);
  io_func(&link->unique_offset, sizeof (link->unique_offset), (size_t) 1, fp);
  link->editscript = gt_editscript_io(link->editscript, fp, io_func);
}

/*generic IO function for one n_r_encseq unique entry*/
static void gt_n_r_encseq_uniqueentry_io(GtNREncseqUnique* unique,
                                         FILE* fp,
                                         EditscriptIOFunc io_func)
{
  gt_assert (unique != NULL);
  io_func(&unique->orig_startpos, sizeof (unique->orig_startpos),
          (size_t) 1,
          fp);
  io_func(&unique->len, sizeof (unique->len), (size_t) 1, fp);
}

/*generic IO function for n_r_encseq data structure*/
static void gt_n_r_encseq_io(GtNREncseq *nre,
                             FILE* fp,
                             NREncseqIOFunc io_func)
{
  GtUword idx;
  io_func(&nre->orig_length, sizeof (nre->orig_length), (size_t) 1, fp);
  io_func(&nre->ldb_allocated, sizeof (nre->ldb_allocated), (size_t) 1, fp);
  io_func(&nre->ldb_nelems, sizeof (nre->ldb_nelems), (size_t) 1, fp);
  io_func(&nre->udb_allocated, sizeof (nre->udb_allocated), (size_t) 1, fp);
  gt_assert(nre->udb_allocated > 0);
  io_func(&nre->udb_nelems, sizeof (nre->udb_nelems), (size_t) 1, fp);
  gt_assert(nre->udb_nelems > 0);
  if (nre->links == NULL) {
    nre->links = gt_malloc(sizeof (*nre->links) * nre->ldb_nelems );
    for (idx = 0; idx < nre->ldb_nelems; idx++) {
      nre->links[idx].editscript = NULL;
    }
  }
  for (idx = 0; idx < nre->ldb_nelems; idx++) {
    gt_n_r_encseq_linkentry_io(&nre->links[idx], fp, io_func);
  }
  if (nre->uniques == NULL) {
    nre->uniques = gt_malloc(sizeof (*nre->uniques) * nre->udb_nelems );
  }
  for (idx = 0; idx < nre->udb_nelems; idx++) {
    gt_n_r_encseq_uniqueentry_io(&nre->uniques[idx], fp, io_func);
  }
}

/*write n_r_encseq data structure to file*/
void gt_n_r_encseq_write_nre(GtNREncseq* nre,
                             FILE* fp)
{
  gt_assert(nre != NULL && fp != NULL);
  gt_n_r_encseq_io(nre, fp, gt_n_r_encseq_gt_xfwrite);
}
...
GtNREncseq *gt_n_r_encseq_read_nre(const char *basename_nre,
                                   ...
                                   GtError *err)
{
  ...
  if (!had_err) {
    ...
    if (fp == NULL) {
      ...
      gt_n_r_encseq_io(nre, fp, gt_n_r_encseq_gt_xfread);
      ...
      /*iterate through link entrys and store ids in corresponding unique entry
        array*/
                                                     ...
                                                     GtNREncseqCompressor *nrec)
{
  ...
  nrec->current_seq_start = gt_encseq_seqstartpos(nrec->input_es,
                                                  nrec->main_seqnum);
  gt_assert(nrec->current_seq_start ==
            gt_intset_get_idx_smaller_geq(nrec->nre->ssptab,
                                          nrec->main_seqnum));
...
int gt_n_r_encseq_compressor_compress(GtStr *basename,
                                      GtNREncseqCompressor *n_r_e_compressor,
                                      GtEncseq *encseq,
                                      GtError *err)
{
  ...
  if (!had_err) {
    gt_n_r_encseq_write_nre(n_r_e_compressor->nre, fp);
    gt_fa_xfclose(fp);
    fp = NULL;
    fp = gt_fa_fopen_with_suffix(gt_str_get(basename), ".fas",
                                 "w", err);
    if (fp == NULL)
      had_err = -1;
  }
