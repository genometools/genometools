...
/* Compare two keys.  */
static int cmp_fn (const Keytype a,const Keytype b,UNUSED void *info)
{
  unsigned long va, vb;
      ...
      case Build:
        if (i < (unsigned long) SIZE)
        {
          if (rbt_find (xtab + j, *root, cmp_fn, NULL) !=
              NULL)
          {
            error_set(err,"Found element which is not in tree yet");
            haserr = true;
          }
          if (!haserr)
          {
            ...
            if (elem == NULL || rbt_find (xtab + j, *root, cmp_fn,
                                                  NULL) == NULL)
            {
              error_set(err,"Couldn't find element after it was added");
              haserr = true;
            }
          }
        }
      ...
      case Delete:
        ...
        if (elem == NULL
            || rbt_delete (xtab + j, root, cmp_fn,
                                   NULL) != 0)
        {
          error_set(err,"Error deleting element");
          haserr = true;
        }
        break;
