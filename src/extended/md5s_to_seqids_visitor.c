...
#include "extended/md5s_to_seqids_visitor.h"
...
#define  md5s_to_seqids_visitor_cast(GV)\
         gt_node_visitor_cast(gt_md5s_to_seqids_visitor_class(), GV)

static void md5s_to_seqids_visitor_free(GtNodeVisitor *nv)
{
  GtMD5ToSeqidsVisitor *md5s_to_seqids_visitor =
    md5s_to_seqids_visitor_cast(nv);
  gt_assert(md5s_to_seqids_visitor);
  gt_region_mapping_delete(md5s_to_seqids_visitor->region_mapping);
}
...
static void m2s_build_new_target(GtStr *target, GtStrArray *target_ids,
...
static int m2s_change_target_seqids(GtGenomeNode *gn, const char *target,
                                GtRegionMapping *region_mapping, GtError *err)
{
  ...
  if (!had_err) {
    ...
    m2s_build_new_target(new_target, target_ids, target_ranges, target_strands);
...
static int m2s_change_seqid(GtGenomeNode *gn, void *data, GtError *err)
{
  const char *target;
  M2SChangeSeqidInfo *info = (M2SChangeSeqidInfo*) data;
  gt_error_check(err);
  gt_assert(gn && info);
  gt_genome_node_change_seqid(gn, info->new_seqid);
  if ((target = gt_feature_node_get_attribute((GtFeatureNode*) gn,
                                              TARGET_STRING))) {
    return m2s_change_target_seqids(gn, target, info->region_mapping, err);
  }
  return 0;
}
  ...
  if (gt_md5_seqid_has_prefix(gt_str_get(seqid))) {
    ...
    if (!had_err) {
      ...
      if (gt_feature_node_try_cast(gn)) {
        M2SChangeSeqidInfo info;
        info.new_seqid = new_seqid;
        info.region_mapping = region_mapping;
        had_err = gt_genome_node_traverse_children(gn, &info, m2s_change_seqid,
                                                   true, err);
      }
      else
...
static int md5s_to_seqids_visitor_feature_node(GtNodeVisitor *nv,
                                              GtFeatureNode *fn,
                                              GtError *err)
{
  GtMD5ToSeqidsVisitor *v = md5s_to_seqids_visitor_cast(nv);
  gt_error_check(err);
  return md5_to_seqid((GtGenomeNode*) fn, v->region_mapping, err);
}

static int md5s_to_seqids_visitor_region_node(GtNodeVisitor *nv,
                                             GtRegionNode *rn,
                                             GtError *err)
{
  GtMD5ToSeqidsVisitor *v = md5s_to_seqids_visitor_cast(nv);
...
const GtNodeVisitorClass* gt_md5s_to_seqids_visitor_class()
{
  static const GtNodeVisitorClass *nvc = NULL;
  if (!nvc) {
    nvc = gt_node_visitor_class_new(sizeof (GtMD5ToSeqidsVisitor),
                                    md5s_to_seqids_visitor_free,
                                    NULL,
                                    md5s_to_seqids_visitor_feature_node,
                                    md5s_to_seqids_visitor_region_node,
                                    NULL);
  }
  return nvc;
}

GtNodeVisitor* gt_md5s_to_seqids_visitor_new(GtRegionMapping *region_mapping)
{
  GtNodeVisitor *nv;
  GtMD5ToSeqidsVisitor *md5s_to_seqids_visitor;
  nv = gt_node_visitor_create(gt_md5s_to_seqids_visitor_class());
  md5s_to_seqids_visitor = md5s_to_seqids_visitor_cast(nv);
  md5s_to_seqids_visitor->region_mapping = region_mapping;
  return nv;
}
