...
void graphics_draw_colored_text(Graphics *g, double x, double y, Color col,
                                const char *txt)
{
  assert(g && g->c_class && txt);
...
void graphics_draw_arrowhead(Graphics *g, double x, double y, Color col,
...
int graphics_save_to_file(const Graphics *g, const char *filename, Error *e)
{
  assert(g && g->c_class);
  return g->c_class->save_to_file(g, filename, e);
}
