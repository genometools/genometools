...
int gt_track_unit_test(GtError *err)
{
  ...
  gt_ensure(gt_track_get_number_of_lines(track) == 0);
  gt_ensure(gt_track_get_height(track, &height, sty, err) == 0);
  gt_ensure(height == t_rest);
  gt_ensure(!gt_error_is_set(err));

  gt_ensure(gt_track_insert_block(track, b[0], err) == 0);
  gt_ensure(!gt_error_is_set(err));
  gt_ensure(gt_track_get_number_of_lines(track) == 1);
  gt_ensure(gt_track_get_height(track, &height, sty, err) == 0);
  gt_ensure(height == t_rest + l_rest + BAR_HEIGHT_DEFAULT);
  gt_ensure(!gt_error_is_set(err));

  gt_ensure(gt_track_insert_block(track, b[1], err) == 0);
  gt_ensure(!gt_error_is_set(err));
  gt_ensure(gt_track_get_number_of_lines(track) == 1);
  gt_ensure(gt_track_get_height(track, &height, sty, err) == 0);
  gt_ensure(height == t_rest + l_rest + BAR_HEIGHT_DEFAULT);
  ...
  gt_ensure(gt_track_insert_block(track, b[2], err) == 0);
  gt_ensure(!gt_error_is_set(err));
  gt_ensure(gt_track_get_number_of_lines(track) == 2);
  gt_ensure(gt_track_insert_block(track, b[3], err) == 0);
  gt_ensure(!gt_error_is_set(err));
  gt_ensure(gt_track_get_number_of_lines(track) == 2);
  gt_ensure(gt_track_get_height(track, &height, sty, err) == 0);
  gt_ensure(height == t_rest + 2*(l_rest + BAR_HEIGHT_DEFAULT));
  gt_ensure(!gt_error_is_set(err));

  gt_style_set_num(sty, "exon", "bar_height", 42);
  gt_ensure(gt_track_get_height(track, &height, sty, err) == 0);
  gt_ensure(height == t_rest + 2*(l_rest+42));
  gt_ensure(!gt_error_is_set(err));
  ...
  gt_ensure(gt_track_get_height(track, &height, sty, err) == 0);
  gt_ensure(height == t_rest + 2*(l_rest+23));
  gt_ensure(!gt_error_is_set(err));
  gt_style_unset(sty, "gene", "bar_height");
  gt_style_set_num(sty, "format", "bar_height", 99);
  gt_ensure(gt_track_get_height(track, &height, sty, err) == 0);
  gt_ensure(height == t_rest + 2*(l_rest+99));
  gt_ensure(!gt_error_is_set(err));

  gt_ensure(gt_track_get_number_of_discarded_blocks(track) == 0);
