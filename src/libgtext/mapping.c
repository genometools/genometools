                     ...
                     const char *input, Env *env)
{
  ...
  if (lua_isnil(m->L, -1)) {
    env_error_set(env, "%s[%s] is nil (defined in \"%s\")", m->global,
                  input, str_get(m->mapping_file));
    had_err = -1;
  }
  if (!had_err) {
    switch (m->type) {
      case MAPPINGTYPE_STRING:
        ...
        if (!(lua_isstring(m->L, -1))) {
          env_error_set(env, "%s[%s] is not a string (defined in \"%s\")",
                        m->global, input, str_get(m->mapping_file));
          had_err = -1;
        }
        if (!had_err)
      ...
      case MAPPINGTYPE_INTEGER:
        ...
        if (!(lua_isnumber(m->L, -1))) {
          env_error_set(env, "%s[%s] is not an integer (defined in \"%s\")",
                        m->global, input, str_get(m->mapping_file));
          had_err = -1;
        }
        if (!had_err)
                        ...
                        const char *input, Env *env)
{
  ...
  if (lua_pcall(m->L, 1, 1, 0)) {
    env_error_set(env, "running function '%s': %s", m->global,
                  lua_tostring(m->L, -1));
    had_err = -1;
  }
  ...
  if (!had_err) {
    switch (m->type) {
      case MAPPINGTYPE_STRING:
        if (!lua_isstring(m->L, -1)) {
          env_error_set(env, "function '%s' must return a string (defined in "
       ...
       case MAPPINGTYPE_INTEGER:
        if (!lua_isnumber(m->L, -1)) {
          env_error_set(env, "function '%s' must return an integer) (defined "
                             "in \"%s\")", m->global, str_get(m->mapping_file));
          had_err = -1;
        }
        if (!had_err)
          *integeroutput = lua_tointeger(m->L, -1);
                       ...
                       const char *input, Env *env)
{
  ...
  assert((m->type == MAPPINGTYPE_STRING  && stroutput) ||
         (m->type == MAPPINGTYPE_INTEGER && integeroutput));
  if (m->is_table)
    return map_table(m, stroutput, integeroutput, input, env);
  return map_function(m, stroutput, integeroutput, input, env);
}

Str* mapping_map_string(Mapping *m, const char *input, Env *env)
{
  Str *output = NULL;
  env_error_check(env);
  map_generic(m, &output, NULL, input, env);
  return output;
}

int mapping_map_integer(Mapping *m, long *output, const char *input, Env *env)
{
  env_error_check(env);
