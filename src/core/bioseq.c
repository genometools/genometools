...
static void add_fingerprints(GtStrArray *md5_fingerprints, GT_Bioseq *bs)
{
  unsigned long i;
                                  ...
                                  GT_Error *err)
{
  ...
  if (!bs->use_stdin) {
    gt_bioseq_files_info.gt_bioseq_index =
      gt_xfopen((const char *) gt_str_get(gt_bioseq_index_file), "w");
    gt_bioseq_files_info.gt_bioseq_raw =
      gt_xfopen(gt_str_get(gt_bioseq_raw_file), "w");
  }
...
static GT_Bioseq* gt_bioseq_new_with_recreate_and_type(GtStr *sequence_file,
                                                 bool recreate,
                                                 GT_FastaReaderType
