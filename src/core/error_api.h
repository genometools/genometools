...
/*
   ...
   Thereby, the actual <GT_Error*> object is used to store the __error message__
   while it is signaled by the return value of the called function, if an error
   occured.

   By convention in __GenomeTools__, the <GT_Error*> object is always passed
   ...
   It is possible to use <NULL> as an <GT_Error*> object, if one is not
   interested in the actual error message.

   Functions which do not get an <GT_Error*> object cannot fail due to a user
   error and it is not necessary to check their return code for an error
   condition.
*/
typedef struct GT_Error GT_Error;

/* Return a new <GT_Error*> object */
...
/* Insert an assertion to check that the error <err> is not set or is <NULL>.
   ...
   <GT_Error*> argument to make sure the error propagation has been coded
   correctly. */
