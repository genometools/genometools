...
static GtUchar seqdelivercharSpecialViatables(const GtEncseq *encseq,
                                              GtEncseqReader *esr,
                                              unsigned long pos)
{
  gt_assert(encseq == esr->encseq);
...
static GtEncseqfunctions encodedseqfunctab[] =
  {
    ...
    { /* GT_ACCESS_TYPE_UCHARTABLES */
      ...
      NFCT(seqdeliverchar,seqdelivercharnospecial),
      NFCT(seqdelivercharspecial,seqdelivercharSpecialViatables),
...
static unsigned long fwdgetnextstopposViatables(const GtEncseq *encseq,
                                                GtEncseqReader *esr,
                                                unsigned long pos)
{
  gt_assert(encseq == esr->encseq);
                              ...
                              unsigned long startpos)
{
  ...
  if (esr->encseq->sat != GT_ACCESS_TYPE_BITACCESS)
  {
    ...
    if (gt_encseq_has_specialranges(esr->encseq))
    {
      ...
      {
        stoppos = revgetnextstopposViaequallength(encseq,startpos);
      }
    } else
    {
      stoppos = 0;
                                  ...
                                  GtReadmode readmode,bool complement)
{
  ...
  {
    gt_encseq_extract2bitenc2(fwd,&ptbe1,encseq,esr,startpos);
