...
function usage()
  io.stderr:write(string.format("Usage: %s scorematrix_file\n", arg[0]))
  io.stderr:write("Parse protein scorematrix_file and show it as C program.\n")
  os.exit(1)
end

if #arg == 1 then
  scorematrix_file = arg[1]
else
  usage()
end

protein_alpha = gt.alpha_new_protein()
scorematrix = gt.scorematrix_read_protein(scorematrix_file)
assert(protein_alpha:size() == scorematrix:get_dimension())

print("#include <limits.h>\n")
print("int main(int argc, char *argv[])")
print("{")
print("  int scorematrix[CHAR_MAX][CHAR_MAX] = {{0}};")
for idx1 = 0, scorematrix:get_dimension()-1 do
  for idx2 = 0, scorematrix:get_dimension()-1 do
    print(string.format("  scorematrix['%s']['%s'] = %d;",
